...
export class EgresoService {
  ...
  async findById(id: number): Promise<ServiceResponse<Egreso | null>> {
    try {
      ...
      if (!egreso) {
        return ServiceResponse.failure("egreso not found", null, StatusCodes.NOT_FOUND)
      }
      return ServiceResponse.success<Egreso>("User found", egreso)
    } catch (ex) {
      const errorMessage = `Error finding egreso with id ${id}:, ${(ex as Error).message}`
      logger.error(errorMessage)
      return ServiceResponse.failure("An error occurred while finding egreso.", null, StatusCodes.INTERNAL_SERVER_ERROR)
    }
  }
